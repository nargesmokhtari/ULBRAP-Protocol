...
reader1_socket = create_server_sender()

# Order of connecting
tag1_socket, tag1_address = reader1_socket.accept()
print(f"tag1: \t {tag1_address}")

supply_socket, supply_address = reader1_socket.accept()
print(f"supply: \t {supply_address}")

attacker_socket, attacker_address = reader1_socket.accept()
...
##########################################
#    Session1: Sharing ID of the nodes
##########################################
...
ids_reader1_json = dumper(ids_reader1)
tag1_socket.sendall(ids_reader1_json.encode('utf-8'))
supply_socket.sendall(ids_reader1_json.encode('utf-8'))
attacker_socket.sendall(str(BalN_bin).encode('utf-8'))

ok = tag1_socket.recv(1024).decode('utf-8')
...
##########################################
#    Session1: MSG1, Sending
##########################################

R0_bin = random_binary_generator(160)
print(f"R0_bin {R0_bin}")
...
tag1_socket.sendall(msg1_json.encode('utf-8'))
attacker_socket.sendall(msg1_json.encode('utf-8'))

ok = tag1_socket.recv(1024).decode('utf-8')
ok = attacker_socket.recv(1024).decode('utf-8')

print(f"Sent: Msg 1, session 1:")
print(msg1)
time.sleep(stop_time)

##########################################
#    Session1: MSG2, Recieving
##########################################

# msg2
msg2_data = postman(tag1_socket.recv(1024).decode('utf-8'))
tag1_socket.sendall("ok".encode('utf-8'))
